<?php 
    $stmt3 = $pdo_conn->prepare("SELECT * FROM admin where id_admin=". $_GET["id"]);
    $stmt3->execute();
    $result3 = $stmt3->fetchAll();
    $stmt4 = $pdo_conn->prepare("SELECT * FROM user where username='". $result3[0]['username_admin']."'");
    $stmt4->execute();
...
if(isset($_POST['submit'])){
      ...
      $pdo_statement1 = $pdo_conn->prepare("update admin set nama_admin='" . $_POST[ 'nama' ]. "', no_hp_admin=" . $_POST[ 'hp' ]. " where username_admin='" . $result4[0]['username']."'");
              ...
              <form role="form" id="templatemo-preferences-form" method="post">
                <div class="row">
                  <div class="col-md-6 margin-bottom-15">
                    ...
                    <input type="text" class="form-control" name="nama" required value="<?php echo $result3[0]['nama_admin'];?>">                  
                  </div>
                  <div class="col-md-6 margin-bottom-15">
                    <label for="hp" class="control-label">No HP</label>
                    <input type="text" class="form-control" name="hp" required value="<?php echo $result3[0]['no_hp_admin'];?>">                  
                  </div>
                </div>
                  <div class="row">
